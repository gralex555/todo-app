...
При каждом push в `main` и при создании Pull Request в `main`.

### Что проверяется

1. Скачивание кода (actions/checkout)
2. Установка Java 25 (Eclipse Temurin) с кэшированием Maven-зависимостей
3. Компиляция проекта (./mvnw clean compile)
4. Запуск всех тестов (./mvnw test)

### Инфраструктура для тестов

В CI поднимаются service containers с PostgreSQL и Redis — это позволяет
тестам работать с реальной инфраструктурой, как в production:

- PostgreSQL 16 на порту 5432
- Redis 7 на порту 6379

Kafka в тестах замокана через @MockitoBean.
...
Статус CI отображается:
- В разделе **Actions** репозитория
- На странице каждого Pull Request

### Публикация Docker-образа

После merge в `main` Docker-образ автоматически собирается и публикуется
в **GitHub Container Registry** (ghcr.io).

#### Использование

Скачать и запустить готовый образ:

    docker pull ghcr.io/gralex555/todo-app:latest
    docker run -p 8080:8080 ghcr.io/gralex555/todo-app:latest

#### Теги образов

При каждой публикации создаются два тега:
- `latest` — последняя версия из main
- `<commit-sha>` — конкретный коммит, для точной идентификации версии
